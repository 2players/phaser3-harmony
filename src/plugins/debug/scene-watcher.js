...
export default class SceneWatcherPlugin extends Phaser.Plugins.BasePlugin {
  ...
  getOutput() {
    const sceneTitle = '[ Scene Watcher ]'
    const sceneHeadline = this.getSceneHeadline()
    const sceneInfo = this.game.scene.scenes.map(this.getSceneOutput, this)

    const outputs = [].concat(sceneTitle, sceneHeadline, sceneInfo)
    return outputs.join(NEWLINE)
  }

  getSceneHeadline() {
    return (
      Pad('key', 12, SPACE, PAD_LEFT) +
      SPACE +
      ICON_DEFAULT +
      SPACE +
      Pad('status', 8, SPACE, PAD_RIGHT) +
      Pad('displayList', 12, SPACE, PAD_LEFT) +
      Pad('updateList', 12, SPACE, PAD_LEFT)
    )
  }

  getSceneOutput(scene) {
    return (
      Pad(scene.sys.settings.key.substr(0, 12), 12, SPACE, PAD_LEFT) +
      SPACE +
      getIcon(scene) +
      SPACE +
      Pad(getStatus(scene), 8, SPACE, PAD_RIGHT) +
      Pad(getDisplayListLength(scene), 12, SPACE, PAD_LEFT) +
      Pad(getUpdateListLength(scene), 12, SPACE, PAD_LEFT)
    )
  }
